...
void RunPUPPI::Init()
{
  ...
  fPuppi = new PuppiContainer(fApplyCHS, fUseExp, fMinPuppiWeight, puppiAlgo);
}
...
void RunPUPPI::Process()
{
  ...
  while((candidate = static_cast<Candidate *>(fItTrackInputArray->Next())))
  {
    ...
    if((!fDoGenPUPPI and candidate->IsRecoPU and candidate->Charge != 0) || (fDoGenPUPPI and candidate->IsPU and candidate->Charge != 0))
    { // if it comes fromPU vertexes after the resolution smearing and the dZ matching within resolution
      ...
      curRecoObj.dZ = candidate->Position.Z() - PVZ;
    }
    else if((!fDoGenPUPPI && !candidate->IsRecoPU && candidate->Charge != 0) || (fDoGenPUPPI and !candidate->IsPU and candidate->Charge !=0))
    {
  ...
  // save puppi weights to output collection
  std::vector<double> pWeights = fPuppi->getPuppiWeights();
  for(unsigned icand = 0; icand < InputParticles.size(); ++icand)
  {
    candidate = static_cast<Candidate *>(InputParticles.at(icand)->Clone());
    candidate->PuppiWeight = pWeights[icand];
    //std::cout << "Puppi weight " << candidate->PuppiWeight << std::endl;
