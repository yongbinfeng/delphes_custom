  ...
  for(itTowerHits = fTowerHits.begin(); itTowerHits != fTowerHits.end(); ++itTowerHits)
  {
    ...
    if(flags & 1)
    {
      ...
      if(fTrackFractions[number] > 1.0E-9)
      {
        ...
        fTrackEnergy += energy;
        //std::cout << "track is PU " << track->IsPU << std::endl;
        if (track->IsPU) {
            fTrackEnergyPU += energy;
        } else {
            fTrackEnergyLV += energy;
        }
    ...
    fTowerEnergy += energy;
    //std::cout << "particle is PU " << particle->IsPU << std::endl;
    if (particle->IsPU) {
        fTowerEnergyPU += energy;
    } else {
        fTowerEnergyLV += energy;
    }
...
void SimpleCalorimeter::FinalizeTower()
{
  ...
  fTower->ETruth = fTowerEnergy;
  fTower->ETruthLV = fTowerEnergyLV;
  fTower->ETruthPU = fTowerEnergyPU;
  ...
  if(neutralEnergy > fEnergyMin && neutralSigma > fEnergySignificanceMin)
  {
    ...
    tower->ETruth = fTowerEnergy - fTrackEnergy;
    tower->ETruthLV = fTowerEnergyLV - fTrackEnergyLV;
    tower->ETruthPU = fTowerEnergyPU - fTrackEnergyPU;
    tower->IsPU = (tower->ETruthLV < 0.5*(tower->ETruth));
    ...
    while((track = static_cast<Candidate *>(fItTowerTrackArray->Next())))
    {
      ...
      track->ETruth = track->Momentum.E();
      if (fabs(track->Momentum.E())<0.01) {
        std::cout << "found track with 0 energy: Pt : "<< track->Momentum.Pt() << " Eta " << track->Momentum.Eta() << " Phi "<< track->Momentum.Phi() << " M " << track->Momentum.M() << " energy " << track->Momentum.E() << std::endl;
      }
      if (track->IsPU) {
        track->ETruthPU = track->Momentum.E();
        track->ETruthLV = 0.;
      } else {
        track->ETruthPU = 0.;
        track->ETruthLV = track->Momentum.E();
      }
