...
void TreeWriter::ProcessParticleFlowCandidates(ExRootTreeBranch *branch, TObjArray *array)
{
  ...
  while((candidate = static_cast<Candidate *>(iterator.Next())))
  {
    ...
    entry->PID = candidate->PID;
    entry->PdgID = candidate->PID;
    if ((candidate->Charge!=0) && (fabs(candidate->PID)!=11) && (fabs(candidate->PID)!=13)) {
        entry->PdgID = (candidate->Charge > 0) ? 211 : -211; // at reco level, for charged hadrons, can only assign pions to it.
    }
